...
import { LIBRARY_GATES, LIBRARY_STORIES } from "./libraryGateScripts";
...
describe("图书馆关卡脚本 · 红线校验", () => {
  ...
  it("全量 10 关，且每关都要求玩家自己写英文（红线 1）", () => {
    expect(LIBRARY_GATES).toHaveLength(10);
    for (const gate of LIBRARY_GATES) {
  ...
  it("LIBRARY_STORIES 与 LIBRARY_GATES 一一对应", () => {
    expect(LIBRARY_STORIES).toHaveLength(LIBRARY_GATES.length);
  });
