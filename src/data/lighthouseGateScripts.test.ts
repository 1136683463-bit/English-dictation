...
import { LIGHTHOUSE_GATES, LIGHTHOUSE_STORIES } from "./lighthouseGateScripts";
...
const ALL_NPC_LINES = LIGHTHOUSE_GATES.flatMap((gate) => [
...
describe("灯塔关卡脚本 · 红线校验", () => {
  ...
  it("剧情 setup 不含禁用词（红线 3 覆盖剧情文本）", () => {
    for (const story of LIGHTHOUSE_STORIES) {
      for (const pattern of FORBIDDEN_PATTERNS) {
  ...
  it("LIGHTHOUSE_STORIES 与 LIGHTHOUSE_GATES 一一对应", () => {
    expect(LIGHTHOUSE_STORIES).toHaveLength(LIGHTHOUSE_GATES.length);
  });
