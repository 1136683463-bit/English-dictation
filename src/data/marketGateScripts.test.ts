...
import { MARKET_GATES, MARKET_STORIES } from "./marketGateScripts";
...
describe("集市关卡脚本 · 红线校验", () => {
  ...
  it("剧情 setup 不含禁用词（红线 3 覆盖剧情文本）", () => {
    for (const story of MARKET_STORIES) {
      for (const pattern of FORBIDDEN_PATTERNS) {
  ...
  it("MARKET_STORIES 与 MARKET_GATES 一一对应", () => {
    expect(MARKET_STORIES).toHaveLength(MARKET_GATES.length);
  });
